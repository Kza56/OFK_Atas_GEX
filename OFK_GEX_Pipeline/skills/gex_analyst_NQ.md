# Skill: GEX Analyst NQ — Intraday Scalping Briefing for NQ E-mini

## Instruction
Read the file full_levels_NQ.json (path configured in config.py, defaults to
OFK_GEX_Pipeline/data/full_levels_NQ.json) and generate the briefing
according to the schema below. Reply with VALID JSON ONLY, nothing else.

## Permanent context — focus INTRADAY SCALPING
- Instrument        : NQ E-mini Nasdaq-100 (CME, $20/point)
...
- **dex_plus_intraday_nq**: strike with maximum positive net delta
                             → dealers BUY the underlying (bullish hedging)
- **dex_minus_intraday_nq**: strike with maximum negative net delta
...
- **expected_move_tt_nq**: straddle-weighted EM (ATM_straddle × 0.6 + OTM1 × 0.3 + OTM2 × 0.1)
...
- expected_move_iv_nq: legacy IV-based formula (fallback)
- range_high_nq / range_low_nq now use TT if available, otherwise IV.

### 0DTE levels (very relevant in afternoon / end of session)
- **max_pain_0dte_nq**     : Max Pain computed on options expiring today
                              (or tomorrow if no expiration today)
- **pin_strike_0dte_nq**   : strike with maximum |gamma|×OI 0DTE = strongest magnet
- **charm_magnet_0dte_nq** : OTM strike closest to spot (with OI > 5%)
...
- max_pain_nq, expected_move_nq, range_low_nq, range_high_nq, pcr

### Market context _meta (NEW — VIX + macro + pipeline health)
The `_meta` block in full_levels_NQ.json contains:
...
- **buy_zone** = p_trans_intraday_nq (preferred) else put_wall_intraday_nq else range_low_nq
- **sell_zone** = c_trans_intraday_nq (preferred) else call_wall_intraday_nq else range_high_nq
- **bullish_invalidation** = call_wall_intraday_nq — beyond = bullish squeeze
- **bearish_invalidation** = put_wall_intraday_nq — below = bearish squeeze

### If gex_regime = 1 but term in BACKWARDATION (immediate stress)
- **buy_zone** = range_low_nq (= TastyTrade EM low if available) — wide stops
- **sell_zone** = range_high_nq (= TastyTrade EM high if available)
...
- **bullish_invalidation** = call_wall_intraday_nq else call_wall (structural)
- **bearish_invalidation** = put_wall_intraday_nq else put_wall

### DEX override (high priority if near spot)
- If dex_plus_intraday_nq is between spot and initial sell_zone (<50% of distance)
  → adjust sell_zone toward dex_plus_intraday_nq (closer target).
- If dex_minus_intraday_nq is between spot and initial buy_zone
  → adjust buy_zone toward dex_minus_intraday_nq.

### Auxiliary levels
- top_oi_intraday[0].strike_nq can serve as secondary support/resistance
- charm_magnet_0dte_nq near spot → afternoon scalp target
