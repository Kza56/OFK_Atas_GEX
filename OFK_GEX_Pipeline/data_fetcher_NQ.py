...
from config import NQ_LEVELS_JSON
...
def calc_zero_dte_metrics(contracts: list, spot: float) -> dict:
    ...
    for c in zero_dte:
        ...
        by_strike[k]["total_oi"] += c["oi"]
    if by_strike:
        pin = max(by_strike.values(), key=lambda x: x["gex_abs"])
    ...
    if charm_candidates:
        ...
        charm_strike = pin_strike  # fallback

    return {
        "max_pain_0dte_qqq"     : max_pain_strike,
        "pin_strike_0dte_qqq"   : pin_strike,
        "charm_magnet_0dte_qqq" : charm_strike,
...
def build_levels(max_dte: int = 7) -> dict:
    ...
    print("=" * 55)
    print(f"  GEX AGENT — CBOE QQQ fetch  [mode: {mode}]")
    print("=" * 55)
    ...
    NQ_LEVELS_JSON.parent.mkdir(parents=True, exist_ok=True)
    NQ_LEVELS_JSON.write_text(json.dumps(levels, indent=2))
    ...
    if zero_dte.get("max_pain_0dte_qqq"):
    ...
    if iv_id:
        print(f"  IVx intraday  : {iv_id*100:.1f}%  (QQQ {iv_intraday.get('atm_iv_intraday_dte',0)} DTE)")
    ...
    if sk:
        print(f"  Skew 25Δ CBOE : {sk*100:+.2f} vp  (QQQ {skew_cboe.get('skew_cboe_dte',0)} DTE)")
