...
def merge_levels() -> dict:
    ...
    full = {
        ...
        # Call/Put Wall GEX values (CBOE — structurel, all-expirations)
        ...
        **{f"vol_flow_{i}_total": cboe.get(f"vol_flow_{i}_total") for i in range(1, 4)},
        ...
        # CBOE metrics converted to NQ — privilégie l'EM TastyTrade si dispo
    ...
    # IV Rank (Phase 3) — calculé sur l'historique des snapshots, AVANT
    # de sauvegarder le snapshot du jour (sinon self-reference).
    ...
    if full.get("pin_strike_0dte_nq"):
        ...
        print(f"  Charm Mag. 0DTE  : NQ {full['charm_magnet_0dte_nq']}  (proche spot)")
...
if __name__ == "__main__":
    ...
    try:
        import argparse
        p = argparse.ArgumentParser(description="Pipeline NQ matin (CME + CBOE + Claude + PDF)")
        ...
        # NYSE holidays / weekends : on n'a pas de séance → abort sauf override
