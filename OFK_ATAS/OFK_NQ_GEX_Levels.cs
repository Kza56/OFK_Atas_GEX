...
namespace OFK_GEX
{
    ...
    public class OFK_NQ_GEX_Levels : Indicator
    {
        ...
        // (partagés avec OFK_ES_GEX_Levels et OFK_*_ContextScore).
        ...
        public string JsonPath { get; set; } = @"C:\Users\steph\Documents\GitHub\OFK_Atas_GEX\OFK_GEX_Pipeline\data\full_levels_NQ.json";
        ...
        // Désactivés par défaut: les Top OI structurels (toutes expirations) sont
        // souvent à >5% du spot. Préférer les Top OI intraday (section 02b).
        ...
        public string ScriptPath { get; set; } = @"C:\Users\steph\Documents\GitHub\OFK_Atas_GEX\OFK_GEX_Pipeline\run_morning_NQ.py";
        ...
        private void CheckAlerts(int bar)
        {
            ...
            if (AlertCrossGammaFlip && CrossDetected(prevClose, currClose, (decimal)lv.GammaFlip))
                FireAlert("gamma_flip",
                    $"NQ {currClose:0} traverse Gamma Flip {lv.GammaFlip:0} — changement régime gamma",
                    curr);
            ...
            if (AlertCrossWalls)
            {
                if (CrossDetected(prevClose, currClose, (decimal)lv.CallWallIntraday))
                    FireAlert("call_wall_id", $"NQ traverse Call Wall intraday {lv.CallWallIntraday:0}", curr);
                if (CrossDetected(prevClose, currClose, (decimal)lv.PutWallIntraday))
                    FireAlert("put_wall_id", $"NQ traverse Put Wall intraday {lv.PutWallIntraday:0}", curr);
            }

            // 3. cTrans / pTrans (zone gamma)
            if (AlertCrossTrans)
            {
                if (CrossDetected(prevClose, currClose, (decimal)lv.CTransIntraday))
                    FireAlert("c_trans", $"NQ traverse cTrans {lv.CTransIntraday:0} — changement zone gamma", curr);
                if (CrossDetected(prevClose, currClose, (decimal)lv.PTransIntraday))
                    FireAlert("p_trans", $"NQ traverse pTrans {lv.PTransIntraday:0} — changement zone gamma", curr);
            }

            // Alertes prédictives (approche avant cross) — toggles propres aux cross alerts ci-dessus
            ...
            if (AlertPin0DTE && lv.PinStrike0DTE > 0)
            {
                ...
                if (dist <= AlertProximityTicks * tick)
                    FireAlert("pin_0dte",
                        $"NQ proche Pin Strike 0DTE {lv.PinStrike0DTE:0} (±{AlertProximityTicks}t)",
                        curr);
            }

            // 5. Charm Magnet — uniquement dernière heure RTH
            if (AlertCharmMagnet && lv.CharmMagnet0DTE > 0 && IsLastHourRTH())
            {
                ...
                if (dist <= AlertProximityTicks * tick * 3)
                    FireAlert("charm_magnet",
                        $"NQ approche Charm Magnet 0DTE {lv.CharmMagnet0DTE:0} (last hour)",
                        curr);
            }

            // 6. IVR extrême (uniquement si l'historique est suffisant)
            ...
            // 7. Term backwardation aigu (slope > 1 vp)
            ...
            if (AlertVolumeFlowBreach && lv.GexExt1 > 0 &&
                ...
                FireAlert("vol_flow", $"NQ traverse GEX ext-1 {lv.GexExt1:0} — flux directionnel", curr);

            // 11. Macro blackout imminent (<30 min)
            ...
            // 12. Stale / partial / version mismatch (Bloc 7 health checks)
            if (AlertStaleData)
            {
        ...
        {
            // RTH NYSE close = 16:00 ET = 20:00 UTC (EDT) ou 21:00 UTC (EST)
            // Last hour ≈ 15:00-16:00 ET = 19-20 UTC ou 20-21 UTC
        ...
        {
            int filled = (int)Math.Round(pct / 20.0); // 0 à 5 segments
        ...
        private void OpenPanel()
        {
            ...
            // Boutons (sticky bottom, ajoutés DANS L'ORDRE INVERSE car DockPanel empile depuis le bas)
            ...
            // Empiler les éléments en bas du DockPanel : ordre d'ajout = du bas vers le haut
        ...
        protected override void OnRender(RenderContext context, DrawingLayouts layout)
        {
            ...
            // - Au-dessus du Call Wall   = squeeze haussier (jaune)
            // - cTrans → Call Wall       = positive gamma (vert)
            // - pTrans → cTrans          = transition (gris)
            // - Put Wall → pTrans        = negative gamma (rouge)
            // - En-dessous du Put Wall   = squeeze baissier (jaune)
            if (ShowGammaZones)
            {
                ...
                // Helper: clamp Y dans la zone visible
                ...
                // Squeeze haussier : au-dessus du Call Wall (du haut du chart à yCw)
                if (cw > 0)
                {
                    int yCw = Clamp(cw);
                    if (yCw > 0)
                ...
                // Positive gamma : entre cTrans et Call Wall
                if (cw > 0 && ct > 0 && ct < cw)
                {
                    int yCw = Clamp(cw);
                    int yCt = Clamp(ct);
                ...
                // Transition : entre pTrans et cTrans
                if (ct > 0 && pt > 0 && pt < ct)
                {
                    int yCt = Clamp(ct);
                    int yPt = Clamp(pt);
                ...
                // Negative gamma : entre Put Wall et pTrans
                if (pw > 0 && pt > 0 && pw < pt)
                {
                    int yPt = Clamp(pt);
                    int yPw = Clamp(pw);
                ...
                // Squeeze baissier : en-dessous du Put Wall (de yPw au bas du chart)
                if (pw > 0)
                {
                    int yPw = Clamp(pw);
                    if (yPw < chartH)
